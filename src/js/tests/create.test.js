...
const TEST_MEDIA = "https://themenewyork.mystore4.no/users/themenewyork_mystore_no/images/54099_Gucci_demo1111_1.png"
...
function fetchSuccess (){
  return Promise.resolve({
    ...
    status: 200,
    statusText: "OK",
    json: () => Promise.resolve(JSON.parse(TEST_OBJECT)),
  })
}

function fetchError (status = 404, statusText = `"Unauthorized" || "Bad request" || "Not Found"`){
  return Promise.resolve({
    ok: false,
    status,
    statusText
...
describe("createPost", () => {
  it("checks that the create item function creates a new item on the API ", async () => {
    ...
    expect(response).toEqual(JSON.parse(TEST_OBJECT));
  ...
  it ("Throws an error message", async () => {
    ...
    await expect (createPost(
      ...
      ).rejects.toThrow("Unauthorized" || "Bad request" || "Not Found")
  })
});
